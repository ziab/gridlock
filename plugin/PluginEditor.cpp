...
int MidiGridAnalyzerAudioProcessorEditor::indexForTimeSig (int num) noexcept
{
    for (int i = 0; i < (int) std::size (kTimeSigNums); ++i)
        if (kTimeSigNums[i] == num) return i;
    return 2; // 4/4
}
int MidiGridAnalyzerAudioProcessorEditor::timeSigForIndex (int idx) noexcept
{
    if (idx >= 0 && idx < (int) std::size (kTimeSigNums)) return kTimeSigNums[idx];
    return 4;
}
int MidiGridAnalyzerAudioProcessorEditor::barsForIndex (int idx) noexcept
{
    if (idx >= 0 && idx < (int) std::size (kBarsValues)) return kBarsValues[idx];
    return 4;
}
...
void MidiGridAnalyzerAudioProcessorEditor::styleToggle (juce::TextButton &b, juce::uint32 onColour, juce::uint32 offText,
                                                        juce::uint32 onText)
{
...
void MidiGridAnalyzerAudioProcessorEditor::setupControls ()
{
    ...
    styleCombo (clickSubComboBox, {"Off", "1/4 Notes", "1/8 Notes", "1/16 Notes", "Triplets"}, clickSubLabel, "Click Sub:");
    styleCombo (clickSoundComboBox, {"Wood Clave", "Drum Stick Click", "Digital Beep", "Cowbell"}, clickSoundLabel, "Click Sound:");
    ...
    pauseButton.onStateChange = [this] { pauseButton.setButtonText (pauseButton.getToggleState () ? "RESUME" : "PAUSE"); };
    ...
    clearButton.onClick = [this] { eventHistory.clear (); gridComponent.clearEvents (); };
}
...
GridViewState MidiGridAnalyzerAudioProcessorEditor::buildGridViewState (int barsVal) const
{
    ...
    s.numBars = barsVal;
    s.gridSubdivisionPpq = MidiGridAnalyzerAudioProcessor::getSubdivisionPpq (subdivisionComboBox.getSelectedItemIndex ());
...
void MidiGridAnalyzerAudioProcessorEditor::resized ()
{
    ...
    Item items[] = {
        {&barsComboBox, 74}, {&subdivisionComboBox, 74}, {&toleranceSlider, 85},
        {&latencySlider, 85}, {&velocitySlider, 65}, {&bpmSlider, 75},
