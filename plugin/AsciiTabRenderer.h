...
inline bool hasCollision (const std::vector<HitEvent> &events, const double minPpq, const double maxPpq,
                          const double interval, const double latencyPpq) noexcept {
  if (interval <= 0.0 || events.empty ()) {
    return false;
  }

  constexpr int numLanes = 6;
  const int numCols = static_cast<int> (std::ceil ((maxPpq - minPpq) / interval)) + 1;
  if (numCols <= 0 || numCols > kMaxGridCols) {
    return true;
  }
...
inline double selectInterval (const std::vector<HitEvent> &events, const double minPpq, const double maxPpq,
                              const double preferredInterval, const double latencyPpq) noexcept {
  if (preferredInterval > 0.0) {
...
inline std::string formatDeviation (const double deltaMs, const float tol) noexcept {
  if (std::abs (deltaMs) <= tol) {
    return "  \xE2\x9C\x93 "; // ✓ with padding (utf-8)
  }
  char buf[16]{};
...
inline char hitGlyph (const int lane, const size_t count) noexcept {
  ...
  if (count > 1) {
    return (count < 10) ? static_cast<char> ('0' + count) : '*';
  }
  if (lane == 3) {
    return 'o'; // SNARE
  }
...
inline Grid buildGrid (const std::vector<HitEvent> &events, const double minPpq, const double maxPpq,
                       const int totalCols, const double interval, const double latencyPpq) noexcept {
  const auto &lanes = DrumMap::getStandardDrumLanes ();
...
inline void computeAverages (Grid &grid, const double interval, const float bpm, const float tol,
...
inline SystemLayout computeSystemLayout (const int totalCols, const int timeSigNum, const double interval,
                                         const RenderOptions &opts, const int numBars) noexcept {
...
// ── rendering helpers ──
inline std::string buildRuler (const int startCol, const int sysCols, const double minPpq, const double interval,
...
inline std::string buildBeatNumbers (const int startCol, const int sysCols, const double interval,
                                     const int timeSigNum) noexcept {
...
inline std::string buildHitRow (const Grid &grid, const int lane, const int startCol, const int sysCols) noexcept {
...
inline std::string buildDeviationIndicator (const Grid &grid, const int lane, const int startCol, const int sysCols,
...
inline std::string buildVelocityRow (const Grid &grid, const int lane, const int startCol, const int sysCols) noexcept {
  ...
  for (int c = 0; c < sysCols; ++c) {
    ...
    if (v < 10) {
      velRow[c] = static_cast<char> ('0' + v);
    } else if (v < 36) {
      velRow[c] = static_cast<char> ('A' + (v - 10));
    } else {
      velRow[c] = '*';
    }
  }
...
inline std::pair<int, int> countAccuracy (const Grid &grid, const double interval, const float bpm, const float tol,
                                          const double latencyPpq) noexcept {
  int total = 0, green = 0;
...
inline void appendHeader (std::ostringstream &out, const GridViewState &view, const double interval,
                          const RenderOptions &opts, const int totalCols) {
  out << "Gridlock Tab | BPM " << static_cast<int> (std::round (view.bpm)) << " " << view.timeSigNum << "/4"
      << " | Subdiv " << intervalLabel (interval);
  if (opts.fixedIntervalPpq <= 0.0) {
    out << " AUTO";
  }
...
inline void appendSystem (std::ostringstream &out, const Grid &grid, const GridViewState &view,
                          const RenderOptions &opts, const SystemLayout &layout, const double minPpq,
                          const double interval, const int sysIndex) {
  ...
  out << "\n-- System " << (sysIndex + 1) << "/" << layout.numSystems << " Bars " << startBar << "-" << endBar
      << " --\n";
  ...
  for (int lane = 0; lane < numLanes; ++lane) {
    ...
    if (opts.showVelocityRow) {
      ...
      if (hasAnyHit (velRow)) {
        out << std::string (kLabelWidth, ' ') << " " << velRow << "\n";
      }
    }
  }
}
...
inline RenderResult render (const std::vector<HitEvent> &events, const GridViewState &view,
                            const RenderOptions &opts = {}) noexcept {
  RenderResult res;
  if (view.numBars <= 0 || view.bpm <= 0.0f || view.gridSubdivisionPpq <= 0.0) {
    return res;
  }
  ...
  const double interval = detail::selectInterval (events, minPpq, maxPpq, opts.fixedIntervalPpq, latencyPpq);
  ...
  const detail::SystemLayout layout =
      detail::computeSystemLayout (totalCols, view.timeSigNum, interval, opts, view.numBars);
  ...
  out << "\nACCURACY " << accPct << "% (" << greenHits << "/" << totalHits << " on-grid) tol "
      << static_cast<int> (view.toleranceMs) << "ms\n";
