...
juce::Colour GridComponent::getContinuousHitColor (float deltaMs, float toleranceMs, float maxErrorMs) noexcept
{
    ...
    if (deltaMs < 0.0f) // Rush / Early -> Yellow (#FFEA00) -> Orange (#FF9100) -> Electric Red (#FF1744)
    {
        if (t <= 0.5f)
        {
            ...
            return juce::Colour::fromRGB (255, static_cast<juce::uint8> (juce::jmap (k, 234.0f, 145.0f)), 0);
        }
        else
        {
            const float k = (t - 0.5f) / 0.5f;
            return juce::Colour::fromRGB (255, static_cast<juce::uint8> (juce::jmap (k, 145.0f, 23.0f)),
    ...
    else // Drag / Late -> Cyan (#00E5FF) -> Deep Blue (#2979FF) -> Vivid Purple (#D500F9)
    {
        ...
        {
            const float k = t / 0.5f;
            return juce::Colour::fromRGB (static_cast<juce::uint8> (juce::jmap (k, 0.0f, 41.0f)),
                                          static_cast<juce::uint8> (juce::jmap (k, 229.0f, 121.0f)), 255);
        }
        else
        {
            const float k = (t - 0.5f) / 0.5f;
            return juce::Colour::fromRGB (static_cast<juce::uint8> (juce::jmap (k, 41.0f, 213.0f)),
...
void GridComponent::paint (juce::Graphics &g)
{
    ...
    for (int i = 0; i < numLanes; ++i)
    {
        ...
        g.drawText (drumLanes[static_cast<size_t> (i)].label,
                    juce::Rectangle<float> (12.0f, y, labelWidth - 16.0f, laneHeight), juce::Justification::centredLeft,
                    true);
    }
    ...
    if (subdivisionPpq > 0.0)
    {
        ...
        for (double tick = firstTick; tick <= maxPpq + 0.0001; tick += subdivisionPpq)
        {
            ...
            if (isBarBoundary)
            {
                ...
                if (wrappedBarIdx < 0)
                    wrappedBarIdx += barsWindow;
                const int barNumber = wrappedBarIdx + 1;
            ...
            else if (isBeatBoundary)
            {
                ...
                g.drawText (juce::String (beatNumber), juce::Rectangle<float> (x + 3.0f, 0.0f, 30.0f, rulerHeight),
                            juce::Justification::centredLeft, false);
            }
            else
            {
    ...
    const double userLatencyPpq =
        (static_cast<double> (latencyOffsetMsVal) / 1000.0) * (static_cast<double> (bpmVal) / 60.0);
    const float maxErrorMs =
        static_cast<float> ((subdivisionPpq / 2.0) * (60.0 / static_cast<double> (bpmVal)) * 1000.0);
    ...
    for (const auto &event : activeEvents)
    {
        ...
        const juce::Colour fillColour =
            getContinuousHitColor (static_cast<float> (liveDeltaMs), toleranceMsVal, maxErrorMs);

        const auto hitRect =
            juce::Rectangle<float> (hitX - nodeRadius, hitY - nodeRadius, nodeRadius * 2.0f, nodeRadius * 2.0f);
        ...
        else if (absDelta <= toleranceMsVal)
        {
            ...
            g.setColour (juce::Colour (0xff0a0c10));
            g.strokePath (checkPath, juce::PathStrokeType (strokeWidth, juce::PathStrokeType::curved,
        ...
        else if (liveDeltaMs < 0.0f)
        {
            ...
            g.setColour (juce::Colour (0xff0a0c10));
            g.strokePath (arrowPath, juce::PathStrokeType (strokeWidth, juce::PathStrokeType::curved,
                                                           juce::PathStrokeType::rounded));
        }
        else
        {
        ...
        if (displayNoteNumLabels)
        {
            ...
            const auto noteLabelRect = juce::Rectangle<float> (
                hitX - (noteLabelW * 0.5f), hitY - nodeRadius - noteLabelH - 2.0f, noteLabelW, noteLabelH);
        ...
        if (displayMsLabels)
        {
            ...
            const auto labelRect =
                juce::Rectangle<float> (hitX - (labelW * 0.5f), hitY + nodeRadius + 2.0f, labelW, labelH);
    ...
    const juce::String scoreText = juce::String (accuracyPct) + "%  (" + juce::String (greenVisibleHits) + " / " +
                                   juce::String (totalVisibleHits) + " On-Grid)";
