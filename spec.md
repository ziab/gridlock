...
* Protocol (w): client `drill_command` with `action` = `start|hold|too_fast|pause|resume|retry|finish|tolerance|pass_threshold`; start adds `pattern`, `spacing` (0 eighth, 1 triplet, 2 sixteenth), `bpm`, optional `kick`, `tolerance`, and `passThreshold` (0.70–1.00). Server returns `drill_ack` (`accepted`, optional `error`) and broadcasts `drill` snapshots at 10 Hz including state, pattern, bpm, best, attempted, nextBpm, floorBpm, progress, accuracy, passes, blocks, activeSlot, beatsRemaining, missing/wrong/late/extras, automatic, noHits, limitReached, tolerance, passThreshold, requiredPasses, hasBlock, failAccuracy/failExtras/failSequence and history. The companion setup form carries tolerance and pass-bar sliders and remembers the last setup (pattern, spacing, BPM, kick, tolerance, pass bar) in local prefs, restoring it on next open.
