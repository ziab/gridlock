...
* Assessment: complete repetitions containing at least 32 strokes and 10 seconds. Two consecutive blocks with >=pass-bar% expected strokes correct/on-time (drill setting, default 95%, range 70–100%) and <=2% extras, plus a locked hand/kick sequence, confirm a tempo and schedule +3 BPM, bounded to 300. Each assessed block reports a verdict (accuracy/extras/sequence) so both screens can show the bar ("Need ≥X% + locked sequence, 2 in a row") and why the last block failed. Changing the bar mid-drill clears confirmation like a tolerance change; history is keyed by pattern, spacing, kick, tolerance, and pass bar. A hold block (>=85%, <=5% extras) resets both streaks. Two struggle blocks (or six without confirmation) step back to the confirmed tempo but stay automatic so clean playing climbs again; with nothing confirmed yet the starting tempo is held — never auto-dropped below `drill::minBpm` 60 (m: very slow tempos are harder, not easier, and auto-drops to ~48 BPM locked out increases). Manual Hold/Too fast still lock increases until retry. A full repeat announces each transition; one repeat at the new tempo is unscored.
