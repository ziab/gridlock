...
* Assessment: complete repetitions containing at least 32 strokes and 10 seconds. Two consecutive blocks with >=95% expected strokes correct/on-time and <=2% extras confirm a tempo and schedule +3 BPM, bounded to 300. A hold block (>=85%, <=5% extras) resets both streaks. Two struggle blocks (or six without confirmation) step back to the confirmed tempo but stay automatic so clean playing climbs again; with nothing confirmed yet the starting tempo is held — never auto-dropped below `drill::minBpm` 60 (m: very slow tempos are harder, not easier, and auto-drops to ~48 BPM locked out increases). Manual Hold/Too fast still lock increases until retry. A full repeat announces each transition; one repeat at the new tempo is unscored.
...
* Protocol (w): client `drill_command` with `action` = `start|hold|too_fast|pause|resume|retry|finish|tolerance`; start adds `pattern`, `spacing` (0 eighth, 1 triplet, 2 sixteenth), `bpm`, optional `kick` and `tolerance`. Server returns `drill_ack` (`accepted`, optional `error`) and broadcasts `drill` snapshots at 10 Hz including state, pattern, bpm, best, attempted, nextBpm, progress, accuracy, passes, blocks, activeSlot, beatsRemaining, missing/wrong/late/extras, automatic, noHits, limitReached, tolerance and history.
