...
* Defaults: sixteenths at 60 BPM; eighths at 90; triplets at 60. Two bars of count-in after the next barline. Saved confirmed tempos suggest floor(80% of last confirmed), bounded to 40–300. History lives in `Gridlock/drill-history.json` in the user application-data directory, keyed by normalized pattern, spacing, kick mapping, and tolerance.
...
* Match each MIDI note-on to its nearest expected slot (half-subdivision boundary), first hit owns the slot; further hits are extras. Close slots only after compensated time passes the matching window. Missing, wrong hand/kick class, timing errors, and extras are separate counts. Missing notes never shift sequence phase. Timing uses `Timing::compute`; tolerance is min(configured 20 ms default, 15% stroke interval). Scoring uses frozen latency/noise-floor settings and does not depend on editor visibility or history length.
* Controls: Start, Hold tempo, Too fast, Pause/Resume (fresh count-in), Try again, Finish. Silence for two bars pauses. Finish stops the click and retains confirmed/attempted results. Disconnecting the last companion (or missing its 2-second heartbeat for 5 seconds) suspends increases; reconnect gets current session state and does not restart the drill. Changing setup requires finishing the current drill.
