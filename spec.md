...
| `click_subdivision`| Click Subdivision | Choice | Off, 1/4 Notes, 1/8 Notes, 1/16 Notes, Triplets, Sixtuplets | 1/4 Notes |
...
## 10. Adaptive Grouping Drill (w)
* Standalone desktop owns the drill clock, MIDI scoring, and tempo changes; Flutter is a remote controller. VST/AU do not run adaptive tempo. The existing timed practice ramp is mutually exclusive with drills.
* Input: 1–64 R/L/K letters, case-insensitive, whitespace ignored. Every letter occupies one eighth, eighth-triplet, sixteenth, or sixtuplet (6/quarter, e.g. RLRLKK) subdivision; grouping continues across barlines. R/L both match non-kick hits; kick MIDI note is configurable (default `DrumMap::Kick`), pedal hi-hat is ignored. Sticking, accents, flams, rests, and simultaneous strokes are not verified/supported.
* Defaults: sixteenths at 60 BPM; eighths at 90; triplets at 60. Start immediately plays the chosen click subdivisions and waits indefinitely for the first matching hand/kick hit; its nearest metronome subdivision becomes the grouping origin (m). No barline or count-in is imposed. Saved confirmed tempos suggest floor(80% of last confirmed), bounded to 60–300 (`drill::minBpm`) (m). History lives in `Gridlock/drill-history.json` in the user application-data directory, keyed by normalized pattern, spacing, kick mapping, and tolerance.
* Assessment: complete repetitions containing at least 32 strokes and 10 seconds. Two consecutive blocks with >=pass-bar% expected strokes correct/on-time (drill setting, default 95%, range 70–100%) and <=2% extras, plus a locked hand/kick sequence, confirm a tempo and schedule +climb BPM (setup-only `step` setting, default 3, range 1–10), bounded to 300. Hesitation never steps back automatically: an earned tempo is kept, holding with increases still armed; only an explicit Too fast drops to the best, floored at the session floor, which is the drill's start tempo (not a hardcoded value). Each assessed block reports a verdict (accuracy/extras/sequence) so both screens can show the bar ("Need ≥X% + locked sequence, 2 in a row"), the floor, and why the last block failed. Changing the bar mid-drill clears confirmation like a tolerance change; history is keyed by pattern, spacing, kick, tolerance, and pass bar. A hold block (>=85%, <=5% extras) resets both streaks. Two struggle blocks (or six without confirmation) step back to the confirmed tempo but stay automatic so clean playing climbs again; with nothing confirmed yet the starting tempo is held — never auto-dropped below `drill::minBpm` 60 (m: very slow tempos are harder, not easier, and auto-drops to ~48 BPM locked out increases). Manual Hold/Too fast still lock increases until retry. A full repeat announces each transition; one repeat at the new tempo is unscored.
* Match each MIDI note-on to its nearest expected slot (half-subdivision boundary), first hit owns the slot; further hits are extras. Close slots only after compensated time passes the matching window. Missing, wrong hand/kick class, timing errors, and extras are separate counts. The sticking cursor advances on each primary received hit, including wrong-class attempts, but never on an empty grid slot or duplicate hit (m). Timing uses `Timing::compute`; tolerance is min(drill override initialized from the main tolerance, 15% stroke interval). Scoring uses frozen latency/noise-floor settings and does not depend on editor visibility or history length.
* Controls: Start, Hold tempo, Too fast, Pause/Resume (listen for a fresh first hit), Try again, Finish. Silence for two bars clears the partial assessment and returns to listening while subdivision clicks keep running (m). Finish stops the click and retains confirmed/attempted results. Disconnecting the last companion (or missing its 2-second heartbeat for 5 seconds) suspends increases; reconnect gets current session state and does not restart the drill. Changing setup requires finishing the current drill.
* Audio ownership: fixed-size engine and command storage; message-thread commands/snapshots cross a mutex which the audio callback only tries, never waits for. Audio buffers split at scheduled tempo boundaries. JSON, history I/O, and widgets stay off the audio thread. Ordinary MIDI pass-through is unchanged.
* Protocol (w): client `drill_command` with `action` = `start|hold|too_fast|pause|resume|retry|finish|tolerance|pass_threshold`; start adds `pattern`, `spacing` (0 eighth, 1 triplet, 2 sixteenth, 3 sixtuplet), `bpm`, optional `kick`, `tolerance`, `passThreshold` (0.70–1.00), and `step` (1–10, setup only, never live). Server returns `drill_ack` (`accepted`, optional `error`) and broadcasts `drill` snapshots at 10 Hz including state, pattern, bpm, best, attempted, nextBpm, floorBpm, progress, accuracy, passes, blocks, activeSlot, beatsRemaining, missing/wrong/late/extras, automatic, noHits, limitReached, tolerance, passThreshold, stepBpm, requiredPasses, hasBlock, failAccuracy/failExtras/failSequence and history. The companion setup form carries tolerance, pass-bar, and climb-size sliders and remembers the last setup (pattern, spacing, BPM, kick, tolerance, pass bar, climb size) in local prefs, restoring it on next open.
* Tests (w): drill engine suites cover progression, missing/duplicate/wrong/late strokes, early first stroke, cross-bar triplets, stage expiry, silence/resume, and disconnect/finish. Processor integration verifies normalized commands, standalone restriction, editor-independent scoring, progression, and MIDI pass-through. Flutter verifies state restoration, command ownership, setup validation, and a compact phone layout.

* Drill usability (m): the phone shows a static pattern reference, not a 10 Hz moving stroke cue; both displays avoid suggesting a time-driven sticking phase. The companion drill launcher is in the upper status bar. Selected eighth/sixteenth/triplet/sixtuplet clicks synchronize the desktop grid through the same processor helper for local and remote controls; a running drill locks those selectors and uses its interval for click scheduling, MIDI timing, and grid rendering. Sixtuplet drills fall back to the 1/16T display grid (numerically identical 1/6 PPQ) when idle. Appending Sixtuplets shifts stored normalized values for the 1/8–Triplets slots, so pre-existing saved states may need one re-pick of the click subdivision.
* Regression checks (m): late/free entry, hand-first and kick-first patterns, gaps preserving the next sticking stroke, automatic re-entry after silence, audible between-beat clicks for all drill subdivisions, triplet grid updates, static phone pattern and upper-bar navigation.

* Live drill tolerance (m): `tolerance` command supplies a validated 5–40 ms override; applies immediately to drill scoring and hit colors without writing `tolerance_ms`. Both drill screens initialize from the main value; Finish discards the override for the next exercise. Effective tolerance retains the 15% interval cap. Changes clear the partial assessment, pending tempo change, and confirmed best (history remains keyed by tolerance), preserving tempo and sequence phase. Snapshots expose configured `toleranceOverride` and effective `tolerance`.
* Sequence recognition (m): continuously match cyclic hand/kick phases on consecutive occupied subdivision slots, independent of beat/bar one. One full matching pattern detects/reacquires the sequence and realigns the sticking cursor; equivalent R/L phases remain indistinguishable. A wrong class or missing slot loses detection and cancels pending increases. Gaps never consume a sticking stroke. Timing errors remain separately scored; detection alone does not imply on-time playing. `sequenceDetected` / `sequenceSeen` snapshots drive static listening/detected/lost indicators on both screens, without a moving phone cue.
