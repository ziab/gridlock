...
* Defaults: sixteenths at 60 BPM; eighths at 90; triplets at 60. Start immediately plays the chosen click subdivisions and waits indefinitely for the first matching hand/kick hit; its nearest metronome subdivision becomes the grouping origin (m). No barline or count-in is imposed. Saved confirmed tempos suggest floor(80% of last confirmed), bounded to 40–300. History lives in `Gridlock/drill-history.json` in the user application-data directory, keyed by normalized pattern, spacing, kick mapping, and tolerance.
* Assessment: complete repetitions containing at least 32 strokes and 10 seconds. Two consecutive blocks with >=95% expected strokes correct/on-time and <=2% extras confirm a tempo and schedule +3 BPM, bounded to 300. A hold block (>=85%, <=5% extras) resets both streaks. Two struggle blocks or six blocks without confirmation return to the confirmed tempo (or 80% of the starting tempo if none) and disable increases. A full repeat announces each transition; one repeat at the new tempo is unscored.
